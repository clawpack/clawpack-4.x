...
Acoustics equations with zero initial conditions,
inflow at x=0, solid wall at x=1.

The density and bulk modulus are specified in [code:setrun.py].

The inflow boundary conditions are set in [code:bc1.f] to be an incoming
wave of strength 0.5*sin(omega*t).  The value of omega is specified in
[code:setrun.py].

Note that after time 2*pi/omega the boundary conditions at the left are 
replaced by extrapolation (outflow) boundary conditions.


    
Example [book/chap7/acouinflow]
...
<dt>[code: bc1.f]
<dd>
This subroutine specifies the boundary conditions, including the inflow
condition at the left boundary.

This is a locally-modified version of the library routine
