...
to demonstrate that this does not approximate the weak solution properly.
This is implemented in [code: rp1noncon.f].

The standard Riemann solver for Burgers' equation is in [code: rp1bu.f].

Compile using 
  make .output -f Makefile.conservative
to get this one.
...
<dt>[code:rp1noncon.f] or [code: rp1bu.f] 
<dd>
